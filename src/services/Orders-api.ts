  ...
  return {
    ...
    productId:
      typeof rawItem.productId === 'string'
        ? rawItem.productId
        : typeof (rawItem.product as Record<string, unknown> | undefined)
            ?._id === 'string'
    ...
    weight:
      typeof rawItem.weight === 'string'
        ? rawItem.weight
        : typeof (rawItem.variant as Record<string, unknown> | undefined)
            ?.weight === 'string'
...
const normalizeOrder = (order: unknown): Order => {
  ...
  const rawShipping = (rawOrder.shippingAddress ?? {}) as Record<
    string,
    unknown
  >;
  const rawShipment = (rawOrder.shipment ?? {}) as Record<string, unknown>;
  const rawPaymentDetails = (rawOrder.paymentDetails ?? {}) as Record<
    string,
    unknown
  >;
  ...
  return {
    ...
    user:
      Object.keys(rawUser).length > 0
        ? {
            ...
            phone:
              typeof rawUser.phone === 'string' ? rawUser.phone : undefined,
          }
        : null,
    ...
    shippingAddress:
      Object.keys(rawShipping).length > 0
        ? {
            fullName:
              typeof rawShipping.fullName === 'string'
            ...
            addressLine2:
              typeof rawShipping.addressLine2 === 'string'
                ...
                : undefined,
            city:
              typeof rawShipping.city === 'string'
                ? rawShipping.city
                : undefined,
            state:
              typeof rawShipping.state === 'string'
                ...
                : undefined,
            postalCode:
              typeof rawShipping.postalCode === 'string'
    ...
    orderStatus: normalizeOrderStatus(
      typeof rawOrder.orderStatus === 'string'
        ? rawOrder.orderStatus
        : undefined,
    ),
    paymentMethod:
      typeof rawOrder.paymentMethod === 'string'
        ...
        ? {
            ...
            gateway:
              typeof rawPaymentDetails.gateway === 'string'
                ...
                : undefined,
            paidAt:
              typeof rawPaymentDetails.paidAt === 'string'
                ? rawPaymentDetails.paidAt
                : null,
            amount: toNumber(rawPaymentDetails.amount),
          }
        : null,
    adminNote:
      typeof rawOrder.adminNote === 'string' ? rawOrder.adminNote : undefined,
    notes: typeof rawOrder.notes === 'string' ? rawOrder.notes : undefined,
    isActive: typeof rawOrder.isActive === 'boolean' ? rawOrder.isActive : true,
    isDeleted: Boolean(rawOrder.isDeleted),
    shipment:
      Object.keys(rawShipment).length > 0
        ? {
            trackingId:
              typeof rawShipment.trackingId === 'string'
                ? rawShipment.trackingId
                : undefined,
            courierName:
              typeof rawShipment.courierName === 'string'
                ? rawShipment.courierName
                : undefined,
            dispatchedAt:
              typeof rawShipment.dispatchedAt === 'string'
                ? rawShipment.dispatchedAt
                : null,
            deliveredAt:
              typeof rawShipment.deliveredAt === 'string'
                ? rawShipment.deliveredAt
                : null,
          }
        : null,
    placedAt: typeof rawOrder.placedAt === 'string' ? rawOrder.placedAt : null,
    createdAt:
      typeof rawOrder.createdAt === 'string' ? rawOrder.createdAt : null,
    updatedAt:
      typeof rawOrder.updatedAt === 'string' ? rawOrder.updatedAt : null,
  };
};
...
const resolveFetchOrdersParams = (
  ...
  Pick<FetchOrdersParams, 'isActive' | 'isDeleted'> => {
  ...
  return {
    ...
    isActive: pageOrParams.isActive,
...
export const fetchAdminOrders = async (
  ...
  const { page, limit, isActive, isDeleted } = resolveFetchOrdersParams(
    pageOrParams,
    limitArg,
    options,
  );
...
export const toggleOrderActiveStatus = async (
  orderId: string,
  isActive: boolean,
): Promise<Order> => {
