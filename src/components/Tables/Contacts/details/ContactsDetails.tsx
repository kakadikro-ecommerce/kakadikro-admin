...
const ContactViewModal: React.FC<ContactViewModalProps> = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="w-full max-w-2xl mx-auto bg-white rounded-[2rem] shadow-2xl flex flex-col max-h-[92vh] border border-[#EFE4D5] overflow-hidden">
        <div className="bg-[#3E2723] px-6 py-3 flex justify-between items-center shrink-0">
          ...
          <button
            onClick={onClose}
            className="px-3 py-1.5 hover:bg-white/10 rounded-lg transition-all text-white/80 text-xs font-semibold"
          >
            X
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-8 custom-scrollbar min-h-0">
          <div className="border-[#EFE4D5] rounded-[1.5rem] p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
              ...
              <div>
                <div className="mb-2">
                  <p className="text-[9px] font-black text-[#A69080] tracking-widest">
                    Submitted
                  </p>
                </div>
